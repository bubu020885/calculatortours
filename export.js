...
var XLSX_SOURCES=["lib/xlsx.full.min.js","https://cdn.sheetjs.com/xlsx-0.20.3/package/dist/xlsx.full.min.js"];
var JSPDF_SOURCES=["lib/jspdf.umd.min.js","https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.2/jspdf.umd.min.js"];
var AUTOTABLE_SOURCES=["lib/jspdf.plugin.autotable.min.js","https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.8.4/jspdf.plugin.autotable.min.js"];

function loadFirstWorking(sources){
...
function ensureXlsx(){
  ...
  return loadFirstWorking(XLSX_SOURCES);
}

function ensureJsPDF(){
  if(window.jspdf&&window.jspdf.jsPDF)return Promise.resolve();
  return loadFirstWorking(JSPDF_SOURCES)
    .then(function(){return loadFirstWorking(AUTOTABLE_SOURCES);});
}

// ---- Excel Export ----
function exportExcel(){
  if(!state.yearDays.length){A.setInfo("Bitte zuerst Jahreskalender generieren.","warning");return;}
  A.setInfo("Excel wird erstellt …","loading");
  ensureXlsx().then(function(){
    if(typeof XLSX==="undefined"){A.setInfo("Excel-Bibliothek konnte nicht geladen werden.","error");return;}
    try{
  ...
  }).catch(function(err){A.setInfo("Excel-Bibliothek konnte nicht geladen werden: "+err.message,"error");});
}

// ---- PDF Export ----
function exportPdf(){
  if(!state.yearDays.length){A.setInfo("Bitte zuerst Jahreskalender generieren.","warning");return;}
  A.setInfo("PDF wird erstellt …","loading");
  ensureJsPDF().then(function(){
    if(!window.jspdf||!window.jspdf.jsPDF){A.setInfo("PDF-Bibliothek konnte nicht geladen werden.","error");return;}
    try{
      ...
      for(var mi=0;mi<12;mi++){
        ...
        var body=(function(mDays){return mDays.map(function(d){
        ...
        doc.autoTable({startY:doc.lastAutoTable.finalY,head:[["Datum","Tag","Feiertag/Ferien","Ausl.","Führ.","Gäste","Std.","Notizen"]],body:body,theme:"striped",headStyles:{fillColor:[63,81,181]},styles:{fontSize:7.5,cellPadding:1.5},columnStyles:{0:{cellWidth:22},1:{cellWidth:10},2:{cellWidth:42},3:{cellWidth:18},4:{cellWidth:12,halign:"center"},5:{cellWidth:14,halign:"right"},6:{cellWidth:16,halign:"right"},7:{cellWidth:40}},margin:{left:10,right:10},didParseCell:(function(mDays){return function(data){if(data.section==="body"){var d=mDays[data.row.index];if(!d)return;if(d.ph)data.cell.styles.fillColor=[254,226,226];else if(d.sh)data.cell.styles.fillColor=[254,243,199];else if(d.dow===0||d.dow===6)data.cell.styles.fillColor=[255,247,237];}};})(mDays)});
      }
  ...
  }).catch(function(err){A.setInfo("PDF-Bibliothek konnte nicht geladen werden: "+err.message,"error");console.error(err);});
}
